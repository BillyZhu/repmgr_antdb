...
//for ADB, this  variable is on behalf of the connection that registered nodes from coodinator (repmgr.conf regist_conninfo)
...
static bool  update_node_record_set_master_by_repmgrd (PGconn *conn, int this_node_id, char *cluster);
...
close_connections()
{
	...
	if (PQstatus(slave_conn) == CONNECTION_OK)
		PQfinish(slave_conn);	
}
...
#ifdef ADB
	maxlen_snprintf(repmgr_schema, "%s%s_%s", DEFAULT_REPMGR_SCHEMA_PREFIX,
			 local_options.cluster_name, local_options.group_name);
#else	
...
#ifdef ADB
	log_info(_("connecting to register server database '%s'\n"),
			 local_options.regist_conninfo);
	my_local_conn = establish_db_connection(local_options.regist_conninfo, true);
#else
	...
	do
	{
		...
		switch (node_info.type)
		{
			...
			{	
...
#ifdef ADB
				slave_conn = 	establish_db_connection(local_options.conninfo, true);										
#endif													

#ifdef ADB
				if ( PQstatus(master_conn) != CONNECTION_OK || PQstatus(slave_conn) != CONNECTION_OK )
#else
				if (PQstatus(master_conn) != CONNECTION_OK)
#endif	
				{
					...
					appendPQExpBuffer(&errmsg,
									  _("unable to connect to master or slave node"));
				...
				//check loop until failover_done=true
				do
				{
...
standby_monitor(void)
{
	 ...
	 */
#ifdef ADB
	if (!check_connection(&slave_conn, "standby", NULL))
#else
...
#endif		
	{
		...
		log_info(_("set local node status\n"));
	 ...
	 * required
	 */
	log_debug(_("manual mode upstream disconnected = (%i) \n"),manual_mode_upstream_disconnected);
	 ...
	 */
	log_debug(_("check upstream connection  (%s), type(%s)\n"),upstream_conninfo, upstream_node_type);
	check_connection(&upstream_conn, upstream_node_type, upstream_conninfo);
	//当和主节点连接不上的时候，开始执行failover操作
	if (PQstatus(upstream_conn) != CONNECTION_OK)
	{
		 ...
		 */
		log_verbose(LOG_DEBUG, "failover config info is: %d\n", local_options.failover);
...
#ifdef ADB
			upstream_node = get_node_info(slave_conn, local_options.cluster_name, upstream_node_id);
#else			
  ...
  continue_monitoring_standby:
	...
	do
	{
		...
		switch (ret)
		{
			case 0:// if not standby，cause error
			{	
				/*
...
#ifdef ADB
	res = PQexec(slave_conn, sqlquery);
#else
	res = PQexec(my_local_conn, sqlquery);
#endif	
	if (PQresultStatus(res) != PGRES_TUPLES_OK)
...
#ifdef ADB
	if (PQsendQuery(my_local_conn, sqlquery) == 0)
#else
	if (PQsendQuery(master_conn, sqlquery) == 0)
#endif		
	{
...
do_master_failover(void)
{
	...
	for (i = 0; i < total_active_nodes; i++)
	{
		/*log_debug(_("checking node %i...: is_visible:%s,type:%s,upstream_node_id:%i,node_info.upstream_node_id:%i\n"), 
			nodes[i].node_id,
			nodes[i].is_visible?"true":"false",
			nodes[i].type,
			nodes[i].upstream_node_id,
			node_info.upstream_node_id
			);
		*/
		log_debug(_("checking node %i...upstream_node_id:%d,node_info upstream_node_id:%d\n"),nodes[i].node_id,nodes[i].upstream_node_id,node_info.upstream_node_id);
		...
		if (nodes[i].upstream_node_id != node_info.upstream_node_id)
			continue;
		log_debug(_("connect to  node %i ..."),nodes[i].node_id);
	...
	log_debug("best candidate node id is %i, local node id is %i\n", best_candidate.node_id, local_options.node);
	...
	if (best_candidate.node_id == local_options.node)
	{
		...
		if (r != 0)
		{
			 ...
			 */
#ifdef ADB
			slave_conn = establish_db_connection(local_options.conninfo, false);
			if (slave_conn != NULL)
#else			 
...
#endif				
			{
				int master_node_id;
#ifdef ADB
				master_conn = get_master_connection(slave_conn,
#else
				master_conn = get_master_connection(my_local_conn,
#endif				
													local_options.cluster_name,
...
#ifdef ADB
		//重新通过coordinator建立起连接
		log_debug(_("reconnect to the regist server.\n"));
		PQfinish(my_local_conn);
		my_local_conn = NULL;
		my_local_conn = establish_db_connection(local_options.regist_conninfo, true);
#else		
		...
		log_notice(_("update repl node status for node id=%i\n"), local_options.node);	
#ifdef ADB	
		if (update_node_record_set_master_by_repmgrd(my_local_conn, node_info.node_id,local_options.cluster_name) == false)
		{
  ...
  /* local node not promotion candidate - find the new master */
	else
	{
		...
		if (master_conn != NULL && master_node_id == failed_master.node_id)
		{
			log_notice(_("Original master reappeared - no action taken, master node id is (%d), failed master node id is (%d)\n"), master_node_id, failed_master.node_id);
...
#endif		
#ifdef ADB	 
		// update update upstream node
			if (update_node_record_status(my_local_conn,
										  local_options.cluster_name,
										  local_options.node,
										  "standby",
										  best_candidate.node_id,
...
set_local_node_status(void)
{
	 ...
	 */
#ifdef ADB
	local_node_status = (is_standby(slave_conn) == 1);
#else
...
check_node_configuration(void)
{
	...
	if (PQntuples(res) == 0)
	{
		...
		sqlquery_snprintf(sqlquery,
						  ...
						  local_options.conninfo);
#ifdef ADB
		if (!PQexec(my_local_conn, sqlquery))	
#else
...
static bool
update_node_record_set_master_by_repmgrd(PGconn *conn, int this_node_id, char *cluster)
{
	PGresult   *res;
	char		sqlquery[QUERY_STR_LEN];
